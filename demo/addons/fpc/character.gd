...
func _process(_delta):
	...
	if Globals.clamp_mouse_speed && mouseInput.length_squared() > Globals.mouse_speed_clamp_sqr:
		mouseInput = mouseInput.normalized() * Globals.mouse_speed_clamp
...
func play_headbob_animation(moving):
	if moving and is_on_floor():
		...
		if !was_playing:
			HEADBOB_ANIMATION.seek(0) # Randomize the initial headbob direction
...
func get_aim_angle(target: Node3D) -> float:
	var player_forward = -HEAD.basis.z.normalized() # Assuming -Z is forward
	var to_target = (target.global_transform.origin - HEAD.global_transform.origin).normalized()
	
	var dot = player_forward.dot(to_target)
	var angle = rad_to_deg(acos(dot)) # Convert from radians to degrees
